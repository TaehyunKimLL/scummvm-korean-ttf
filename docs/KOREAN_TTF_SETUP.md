> **구형 문서.** `korean_ttf.map`/TrueType 로더 기준이며 현재 `hires-text` 브랜치는 `korean_ttf_map`을 읽지 않는다. 현행 사용자 설명서는 소스 트리 안의 **`engines/scumm/HIRES_TEXT_SETUP.md`** 입니다 (이 폴더에도 같은 이름의 옛 요약본이 있으니 주의).
